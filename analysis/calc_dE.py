...
def get_Eqs(state,temp,run, qfep,top,lams):
    ...
    return lambda_int1*E1+lambda_int2*(E2-94.7)
        ...
        try:
            ...
            for state in sorted(state_lamdas.keys()):
                if state=="1-RS":
                    lam = (1.0,0.0)
                    qfep = str(temp)+'/'+str(run)+'/qfep.out'
                    E_RS = get_Eqs(state_lamdas[state],temp,run,qfep,topfile,lam)
                elif state=="2-TS":
                    lam = (0.6,0.4)
                    qfep = str(temp)+'/'+str(run)+'/qfep.out'
                    E_TS = get_Eqs(state_lamdas[state],temp,run,qfep,topfile,lam)
                else:
                    break
            U = E_TS-E_RS
            print(f'U_rs = {U}')
